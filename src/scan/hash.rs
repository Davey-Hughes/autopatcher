...
pub fn calc_sha1(file_path: &str) -> Option<Vec<(String, String)>> {
...
pub fn read_unarchived(file_path: &str) -> Vec<(String, String)> {
    let mut file = fs::File::open(file_path).unwrap();
    let mut hasher = Sha1::new();
    io::copy(&mut file, &mut hasher).unwrap();
    vec![(
        file_path.to_string(),
        string_from_sha1(hasher.finalize().to_vec()),
    )]
}

pub fn read_7z(file_path: &str) -> Vec<(String, String)> {
    ...
    for (k, v) in data {
        let mut hasher = Sha1::new();
        hasher.update(v);
        ret.push((k, string_from_sha1(hasher.finalize().to_vec())));
    }
